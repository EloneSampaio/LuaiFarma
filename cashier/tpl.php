...
<div class="box">
 ...
 <h3><center>From&nbsp;<?php echo $_GET['d1'] ?>&nbsp;To&nbsp;<?php echo $_GET['d2'] ?></h3></center>
	...
	$result = $dbo->prepare("Select  c.type,  SUM(s.amount) AS Total FROM `medicine_list` AS c INNER JOIN `sales_list` AS s ON c.sno = s.sno 
	WHERE STR_TO_DATE(`date`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' GROUP BY type ORDER BY Total DESC");
	$result->execute();
...
?>
			...
			<?php
	$result = $dbo->prepare("Select  SUM(amount) AS new FROM`sales_list` WHERE STR_TO_DATE(`date`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "'");
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
 ...
 <thead>
		...
		<th> Cost of Goods </th>
							...
							<?php
	$result = $dbo->prepare("SELECT s.id, s.name, sum( p.total_bill) AS bill, sum(p.amount_paid) AS paid, sum(p.total_bill)-sum(p.amount_paid) AS Balance FROM `supplier` AS s INNER JOIN `supplier_payment` AS p  ON s.id = p.id WHERE STR_TO_DATE(`date`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "' GROUP BY s.id ORDER BY paid DESC");
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
			...
			<?php
	$result = $dbo->prepare("Select  SUM(amount_paid) AS paid FROM `supplier_payment` WHERE STR_TO_DATE(`date`, '%m/%d/%Y') BETWEEN '" . $start_date . "' AND '" . $end_date . "'");
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
